 ...
 *   Timestamp | Name | City | Journey stage | LinkedIn | Email | WhatsApp | Submissions | Last updated
 *
...
const HEADERS = [
  'Timestamp', 'Name', 'City', 'Journey stage',
...
function doPost(e) {
  ...
  try {
    ...
    const required = ['name', 'city', 'journey', 'linkedin', 'email', 'whatsapp'];
    ...
    // duplicate check — email lives in column F (6)
    ...
    if (existingRow) {
      const count = Number(sheet.getRange(existingRow, 8).getValue()) || 1;
      sheet.getRange(existingRow, 2, 1, 6).setValues([[
        data.name, data.city, data.journey, data.linkedin, email, data.whatsapp,
      ]]);
      sheet.getRange(existingRow, 8, 1, 2).setValues([[count + 1, now]]);
      return respond({ ok: true, duplicate: true });
    }

    sheet.appendRow([
      now, data.name, data.city, data.journey,
